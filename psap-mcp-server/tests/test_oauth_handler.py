...
from psap_mcp_server.src.oauth.handler import SCOPE, OAuth2Handler


class TestOAuth2Handler:
    """Test class for OAuth2Handler."""

    @patch("psap_mcp_server.src.oauth.handler.settings")
    def test_create_oauth_session_without_state(self, mock_settings):
        ...
        with patch("psap_mcp_server.src.oauth.handler.OAuth2Session") as mock_oauth:
            mock_session = Mock()
    ...
    @patch("psap_mcp_server.src.oauth.handler.OAuth2Handler.create_oauth_session")
    ...
    @patch("psap_mcp_server.src.oauth.handler.settings")
    @patch("psap_mcp_server.src.oauth.handler.httpx.post")
    ...
    @patch("psap_mcp_server.src.oauth.handler.OAuth2Handler.introspect_token")
    ...
    @patch("psap_mcp_server.src.oauth.handler.OAuth2Handler.verify_access_token")
...
class TestOAuth2HandlerIntegration:
    ...
    @patch("psap_mcp_server.src.oauth.handler.settings")
    @patch("psap_mcp_server.src.oauth.handler.OAuth2Session")
    @patch("psap_mcp_server.src.oauth.handler.httpx.post")
