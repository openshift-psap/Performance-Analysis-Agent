...
class TestPylogger:
    ...
    @patch("psap_mcp_server.utils.pylogger.structlog")
    @patch("psap_mcp_server.utils.pylogger._LOGGING_CONFIGURED", False)
    def test_get_python_logger_structlog_configuration(self, mock_structlog):
        ...
        # Act
        with patch("psap_mcp_server.utils.pylogger._LOGGING_CONFIGURED", False):
            get_python_logger()
    ...
    def test_get_python_logger_import(self):
        ...
        # Act & Assert
        try:
            import psap_mcp_server.utils.pylogger

            assert psap_mcp_server.utils.pylogger.get_python_logger is not None
        except ImportError as e:
            pytest.fail(f"Module should be importable: {e}")

    # Tests for force_reconfigure_all_loggers()
    @patch("psap_mcp_server.utils.pylogger.get_python_logger")
    ...
    @patch("psap_mcp_server.utils.pylogger._LOGGING_CONFIGURED", True)
    @patch("psap_mcp_server.utils.pylogger.get_python_logger")
    def test_force_reconfigure_resets_global_flag(self, mock_get_logger):
        ...
        import psap_mcp_server.utils.pylogger as pylogger_module
    ...
    @patch("psap_mcp_server.utils.pylogger.logging")
    @patch("psap_mcp_server.utils.pylogger._clear_handlers")
    ...
    @patch("psap_mcp_server.utils.pylogger.logging")
    @patch("psap_mcp_server.utils.pylogger._setup_logger")
