...
```
template-mcp-server/
├── psap_mcp_server/           # Main package directory
│   ├── __init__.py
...
6. **Run the server:**
   ...
   uv run python -m psap_mcp_server.src.main
   ```
...
2. **Run tests with coverage reporting:**
   ```bash
   pytest --cov=psap_mcp_server --cov-report=html --cov-report=term
   ```
...
2. **Type checking with MyPy:**
   ```bash
   mypy psap_mcp_server/
   ```

3. **Docstring validation:**
   ```bash
   pydocstyle psap_mcp_server/ --convention=google
   ```
...
pytest --cov=psap_mcp_server --cov-fail-under=80

# Run security checks
bandit -r psap_mcp_server/
safety check

# Build and test container
docker build -t template-mcp-server .
docker run --rm template-mcp-server python -c "import psap_mcp_server; print('OK')"
```
...
1. **Create tool module:**
   ```python
   # psap_mcp_server/src/tools/your_tool.py
...
1. **Place assets in:** `psap_mcp_server/src/tools/assets/`
2. **Create resource handler in:** `psap_mcp_server/src/tools/`
3. **Register in:** `psap_mcp_server/src/mcp.py`
