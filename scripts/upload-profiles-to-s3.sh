...
#
#   s3://<BUCKET>/<PREFIX>/<accelerator>/<model>/<version>/<filename>.json
#
...
#
# Usage:
#   ./scripts/upload-profiles-to-s3.sh <accelerator> <model> <version> <folder>
#
# Examples:
#   ./scripts/upload-profiles-to-s3.sh H200 deepseek-r1 rhaiis-3.2.5 ~/profiler/deepseek-profiles
#   ./scripts/upload-profiles-to-s3.sh H200 gpt-oss vLLM-0.13.0 /tmp/gpt-oss-traces
#   ./scripts/upload-profiles-to-s3.sh MI300X llama-70b vLLM-0.14.0 ./my-traces
#
# The selected file must be a rank-0 trace, matching either:
#   - "rank0" in the name (e.g. trace_rank0_pid455.json)
#   - positional format trace_<start>_<end>_0_<date>_<time>.json
...
usage() {
    echo "Usage: $0 <accelerator> <model-name> <version> <local-folder>"
    echo ""
    echo "Arguments:"
    echo "  accelerator   Accelerator type      (e.g. H200, MI300X)"
    echo "  model-name    S3 model folder name  (e.g. deepseek-r1, gpt-oss, llama-70b)"
    echo "  version       Version folder name   (e.g. rhaiis-3.2.5, vLLM-0.13.0)"
    echo "  local-folder  Path to directory containing .json trace files"
    echo ""
    echo "Examples:"
    echo "  $0 H200 deepseek-r1 rhaiis-3.2.5 ~/profiler/deepseek-profiles"
    echo "  $0 H200 gpt-oss vLLM-0.13.0 /tmp/gpt-oss-traces"
    echo "  $0 MI300X llama-70b vLLM-0.14.0 ./my-traces"
    echo ""
...
echo "  Destination: s3://$BUCKET/$PREFIX/$ACCELERATOR/$MODEL_NAME/$VERSION/"
echo ""
...
#   1. Explicit rank:  trace_rank0_pid467_range2000-2010.json  (contains "rank0")
#   2. Positional rank: trace_1050_1060_0_20260225_002159.json (3rd field is "0")
is_rank0() {
    local f="$1"
    # Explicit rank0 in filename
    [[ "$f" =~ rank0 ]] && return 0
    # Positional: trace_<start>_<end>_<rank>_<date>_<time>.json
    local base="${f%.json}"
    IFS='_' read -ra parts <<< "$base"
    if [[ ${#parts[@]} -ge 4 && "${parts[0]}" == "trace" && "${parts[3]}" == "0" ]]; then
        return 0
    fi
    return 1
}
...
if [ ${#RANK0_FILES[@]} -eq 0 ]; then
    log_err "No rank-0 trace files found"
    echo "  Expected: filename containing 'rank0' or trace_<start>_<end>_0_<date>_<time>.json"
    echo ""
...
for attempt in $(seq 1 $MAX_RETRIES); do
    ...
    for filename in "${PENDING_FILES[@]}"; do
        ...
        s3_key="${PREFIX}/${ACCELERATOR}/${MODEL_NAME}/${VERSION}/${filename}"
...
if [ $SUCCEEDED -gt 0 ]; then
    log_ok "Uploaded $UPLOAD_FILE to s3://$BUCKET/$PREFIX/$ACCELERATOR/$MODEL_NAME/$VERSION/"
fi
if [ $FAILED -gt 0 ]; then
...
echo ""
echo "Verify with:"
echo "  aws s3 ls s3://$BUCKET/$PREFIX/$ACCELERATOR/$MODEL_NAME/$VERSION/"
echo ""
